...
impl Device {
    ...
    pub fn get_video_input(&self) -> Result<VideoInput, VirtualDeviceError> {
        ...
        let response = ureq::post(format!("http://{}/request.cgi", self.ip).as_str()).send_string(
    ...
    pub fn set_video_input(
        ...
        ureq::post(format!("http://{}/request.cgi", self.ip).as_str()).send_string(&body)?;
    ...
    pub fn volume_up(&mut self) -> Result<VirtualDeviceState, VirtualDeviceError> {
        ureq::post(format!("http://{}/request.cgi", self.ip).as_str())
